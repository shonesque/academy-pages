...
function createProgBarTimeline() {
    ...
    for (var i = 0; i < numSlides - 1; i++) {
        var label = "index" + i;
        tl.to(progBars[i], 6, { width: "100%", ease: Linear.easeNone }, label);
    }
...
function updateProgBars() {
    if (target == 0 && !onDemand) {
        ...
        emptyProgBarsTimer = setTimeout(emptyAllProgBars, 6000);
    }
...
function updateIndex() {
    if (current == (selectorBtns.length - 1)) {
        for (i = 1; i < selectorBtns.length; i++) {
            selectorBtns[i].classList.remove('selected-btn');
        }
    }
